...
function App(): React.ReactElement {
  ...
  function saveComment(e: React.SyntheticEvent) {
    e.preventDefault();
    const fieldsets = document.forms[0].querySelectorAll("fieldset");
    const validity = Array.from(fieldsets).every(
      (i) =>
        (i.children[0] as HTMLInputElement | HTMLTextAreaElement).validity.valid
    );
    if (validity) {
      closePopup();
      saveComments(postId, commentName, email, commentText).then((res) => {
        if (res) {
    ...
    <>
      ...
      <section className={`popup ${isPopupOpened && "popup_opened"}`}>
        <form className="popup__container">
          ...
          <Input
            data={commentName}
            setData={setCommentName}
            type="text"
            name="comment-name"
            text="тему/заголовок комментария"
          />
          <Input
            data={commentText}
            ...
            textArea
          />
          <button className="popup__submit" type="button" onClick={saveComment}>
