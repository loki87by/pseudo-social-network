...
const commentsSlice = createSlice({
  ...
  reducers: {
    addComments: (
      ...
      if (
        !state.find(comment => comment.id === action.payload.comment.id)) {
    ...
    addNewComment: (
      state,
      action: PayloadAction<{
        postId: number, name: string, email: string, body: string, id: number
      }>
    ) => {
      if (
        !state.find(comment => comment.id === action.payload.id)) {
